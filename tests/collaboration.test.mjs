...
sqlite.exec(readFileSync('drizzle/0011_funny_caretaker.sql','utf8'));sqlite.exec(readFileSync('drizzle/0012_funny_the_order.sql','utf8'));sqlite.exec(readFileSync('drizzle/0016_brief_korath.sql','utf8'));
...
const planApi=await moduleFrom(replaceTeam(replaceDB(readFileSync('app/api/plans/route.ts','utf8'))).replace(/import \{\s*missionAccess\s*\} from '@\/db\/mission-access';/,'const missionAccess=globalThis.__access;').replace(/import \{[^}]+\} from '@\/db\/mission-git';/,'const {ensureRepository,readCommit,prepareCommit,ancestry}=globalThis.__missionGit;').replace(/import \{[^}]+\} from '@\/lib\/planning';/,'const {validPlan}=globalThis.__planning;').replace(/import \{\s*boundedBody\s*\} from '@\/lib\/artifacts';/,'const {boundedBody}=globalThis.__uploadUtils;'));
...
assert.equal((await taskContext(null,artAction)).status,401);assert.equal((await taskContext('bob',artAction)).status,409);
...
const linked=await linkedUpload('charlie',linkedId,artContext);assert.equal(linked.status,201,JSON.stringify(linked.data));assert.equal((await linkedUpload('charlie',linkedId,artContext)).status,200);
assert.equal(sqlite.prepare('SELECT status,revision FROM mission_actions WHERE id=?').get(artAction).status,'review');
assert.equal((await actionCall({operation:'accept',id:artAction,eventId:crypto.randomUUID(),mission:newId,revision:3,body:'Cannot skip the linked artifact review.'},'alice')).status,409);
assert.equal((await workshopCall('new-member','accept',undefined,linkedId)).status,403);
assert.equal((await workshopCall('bob','revise',undefined,linkedId)).status,200);assert.equal(sqlite.prepare('SELECT revision FROM mission_actions WHERE id=?').get(artAction).revision,4);
assert.equal((await linkedUpload('charlie',crypto.randomUUID(),artContext)).status,409);
const revisedContext=(await taskContext('charlie',artAction)).data,revisedLinked=crypto.randomUUID();assert.equal((await linkedUpload('charlie',revisedLinked,revisedContext)).status,201);
assert.equal((await workshopCall('bob','accept',undefined,revisedLinked)).status,200);
assert.equal(sqlite.prepare('SELECT status,revision FROM mission_actions WHERE id=?').get(artAction).status,'done');
assert.equal(sqlite.prepare('SELECT status FROM workshop_artifacts WHERE id=?').get(revisedLinked).status,'accepted');
assert.equal(sqlite.prepare("SELECT COUNT(*) n FROM action_events WHERE action_id=? AND kind='accept'").get(artAction).n,1);
assert.equal((await workshopCall('bob','accept',undefined,revisedLinked)).status,200);
