...
const fileApi=await moduleFrom(replaceDB(readFileSync('app/api/workspace/route.ts','utf8')).replace(/import \{\s*missionAccess\s*\} from '@\/db\/mission-access';/,'const missionAccess=globalThis.__access;').replace(/import \{[^}]+\} from '@\/db\/mission-git';/,'const {ensureRepository,readCommit,prepareCommit}=globalThis.__missionGit;').replace(/import \{[^}]+\} from '@\/lib\/workspace-files';/,'const {validWorkspacePath,workspaceSize}=globalThis.__workspaceFiles;'));
